    ...
    internal partial class frmDrvSettings : Form
    {
        ...
        private void frmDrvSettings_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    DialogResult result = MessageBox.Show("Application is not running with admin privileges! Some Features may not work!", "Warning!", MessageBoxButtons.OK);
                    if (result == DialogResult.OK)
                    {
                        DialogResult result1 = MessageBox.Show("Would you like to close the application and run with admin privileges?", "?", MessageBoxButtons.YesNo);
        ...
        {
            Process.Start("https://roast247.eu.org");//website
        }
